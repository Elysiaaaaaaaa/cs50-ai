...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                tmp_cell = (cell[0]+i,cell[1]+j)
                if i == 0 and j == 0:
                    continue
                elif 0 <= cell[0]+i < self.height and 0 <= cell[1]+j < self.width:
                    if tmp_cell in self.moves_made or tmp_cell in self.safes:
                        continue
                    elif tmp_cell in self.mines:
                        count -= 1
                    else:
                        cells.add(tmp_cell)
        print('sentense',cells,count)
        ...
        while flag:
            ...
            mines = set()
            for sentence in self.knowledge:
                print('p', sentence.cells, sentence.count)
    ...
    def make_safe_move(self):
        ...
        """
        ans = self.safes.difference(self.moves_made)
        if len(ans) != 0:
            return ans.pop()
        return None
