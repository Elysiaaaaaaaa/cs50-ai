...
def inherit_prob(people, name, one_gene, two_genes):
    if name not in people:
        return PROBS['gene'][2] * (1 - PROBS['mutation']) + PROBS['gene'][1] * 0.5 + PROBS['gene'][0] * (PROBS['mutation'])
    if name in one_gene:
        return 0.5
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if not mother and not father:
            p *= PROBS['gene'][person_genes]
        else:
            mother_prob = inherit_prob(people, mother, one_gene, two_genes)
            father_prob = inherit_prob(people, father, one_gene, two_genes)
            if person_genes == 0:
                p *= (1 - mother_prob) * (1 - father_prob)
            elif person_genes == 2:
                p *= mother_prob * father_prob
            else:
                p *= (mother_prob * (1 - father_prob) + (1 - mother_prob) * father_prob)
        p *= PROBS['trait'][person_genes][person_trait]
